package com.example.wallnoire.presentation.photos.helper
...
class MovieItemPaginationHelper(private val viewModel: PhotosViewModel) :
    PageKeyedDataSource<Int, PhotoViewItem>() {
    override fun loadInitial(
        params: LoadInitialParams<Int>,
        callback: LoadInitialCallback<Int, PhotoViewItem>
    ) {
        viewModel.page = PhotosViewModel.FIRST_PAGE
        viewModel.getPhotosByQuery(true) { viewState ->
            viewState.getData()?.let { photosViewItem ->
                val nextPage =
                    if (photosViewItem.totalPage > viewModel.page.plus(1)) viewModel.page.plus(1) else null
